...
export async function action({ request, params }) {
  ...
  if (action === "delete" && id !== null) {
    ...
    await deleteDiscount(Data, admin);
    try {
    ...
    return formState.offers.map((offer, index) => (
      <div key={index} style={{ marginTop: "10px", marginBottom: "10px" }}>
        <FormLayout.Group condensed>
          ...
          <Button onClick={() => handleRemoveOffer(index, formState, setFormState)}>
        ...
        <Grid.Cell columnSpan={{ xs: 3, sm: 3, md: 3, lg: 6, xl: 6 }}>
          <Card sectioned>
            ...
            <BlockStack gap="500">
              ...
              <InlineStack gap="500">
                <Button id="products" onClick={() => selectProduct(formState, setFormState)} disabled={formState.type && formState.type !== 'product'}>
                  Select Products
                </Button>
                <Button id="collections" onClick={() => selectCollection(formState, setFormState)} disabled={formState.type && formState.type !== 'collection'}>
                  Select Collections
                </Button>
              </InlineStack>
              <Text></Text>
            ...
            <FormLayout.Group condensed title="Offer Details">
              <LegacyStack horizontal>
                ...
                <RadioButton
                  label={`Buy X Get Y`}
                  checked={formState.subDiscount === "bXgY"}
                  id="bXgY"
                  name="subDiscount"
                  disabled={formState.offerType !== "buyXgetY"}
            ...
            {formState.subDiscount && formState.offers.length < maxOffers && (
              <InlineStack gap="200">
                <Button primary onClick={() => handleAddOffer(maxOffers, formState, setFormState)}>
                  Add Offer
                </Button>
